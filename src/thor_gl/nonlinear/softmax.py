...
def he_softmax1(engine: GLEngine, x, attention_mask, rescale=False, debug=False, sk=None):
...
def he_softmax2(engine: GLEngine, x, attention_mask, rescale=False, debug=False, sk=None):
...
def he_softmax(engine: GLEngine, u, attention_mask, rescale, min_x, max_x, n, l, inv_epsilon, output_alpha, debug, sk):
    """
...
def he_inv(engine: GLEngine, numerator: FheData, denominator: FheData, epsilon: float, alpha: float, delta=1):
    d = 0
...
def he_exp1(engine: GLEngine, enc_x, min_x, max_x, n):
    ...
    for _ in range(int(np.log2(n))):
        exp_x = engine.square_elts(exp_x)        
    return exp_x
