import numpy as np
from .gl import FheData
from desilofhe import GLPlaintext
import torch
...
class ThorModule:
    ...
    def cpu(self):
        if not hasattr(self.engine, "to_cpu"):
            return "'to_cpu' not implemented for this engine"
        for key in self.keys:
            for index in np.ndindex(self.weights[key].shape):
                value = self.weights[key][index]
                if isinstance(value, GLPlaintext):
